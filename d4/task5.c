...
const int permutacjaRozszerzajaca[8] = {0, 4, 3, 2, 3, 5, 4, 1};

int main() {
    char *daneWejsciowe = malloc(100000 * sizeof(char));
    char kluczWejsciowy[9];
    char wektorWejsciowy[13];
    int kluczGlowny[8];
    int wektorInicjujacy[12];
    ...
    if (scanf("%12s", wektorWejsciowy) != 1) {
        free(daneWejsciowe);
        return 1;
    }
    ...
    for (int i = 0; i < 12; i++) {
        wektorInicjujacy[i] = wektorWejsciowy[i] - '0';
    }
    ...
    for (int blok = 0; blok < liczbaUzytecznychBitow; blok += 12) {
        ...
        for (int i = 0; i < 12; i++) {
            blokJawny[i] = daneWejsciowe[blok + i] - '0';
            blokJawny[i] = blokJawny[i] ^ wektorInicjujacy[i];
        }

        int lewaStrona[6], prawaStrona[6];
        
        for (int i = 0; i < 6; i++) {
            lewaStrona[i] = blokJawny[i];
            prawaStrona[i] = blokJawny[i + 6];
        }
        ...
        for (int runda = 1; runda <= liczbaRund; runda++) {
            ...
            int kluczRundy[8];
            for (int i = 0; i < 8; i++) {
                kluczRundy[i] = kluczGlowny[(runda + i) % 8];
            }
        ...
        for (int i = 0; i < 6; i++) {
            wektorInicjujacy[i] = prawaStrona[i];
            printf("%d", prawaStrona[i]);
        }
        for (int i = 0; i < 6; i++) {
            wektorInicjujacy[i + 6] = lewaStrona[i];
